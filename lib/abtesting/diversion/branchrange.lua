local modulename = "abtestingDiversionBranchrange"
...
_M.set = function(self, policy)
    ...
    for i, v in pairs(policy) do
        ...
        local stbranch = range['start'] - offset
        local edbranch = range['end'] + offset
        local left  = policyidx * 2
        local right = policyidx * 2 + 1
        local leftBorder  = left.. ':'..upstream 
        local rightBorder = right..':'..upstream 
        database:zadd(policyLib, stbranch, leftBorder, edbranch, rightBorder)
